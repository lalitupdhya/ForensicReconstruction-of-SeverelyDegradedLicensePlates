...
font = ImageFont.truetype("E:\\license\\BebasNeueBold.ttf", 85)
...
for r in range(1, rtc+1):
    for k in range(50):  # number of license plates per districts
        ...
        center = (int(0.5 * 400), int(0.5 * 100))
        yellow = (0, 0, 255, 255)
        ImageDraw.floodfill(pil_img, xy=center, value=yellow)

        draw = ImageDraw.Draw(pil_img)

        number_plate = number_plate_1 + ' ' + number_plate_2
        draw.text((10, 20), number_plate, font=font)
